...
- **不采集/不上传数据**：FViewer 不包含遥测上报逻辑；你加载的数据仅在本地进程内使用。
- **数据默认不落盘**：程序使用内存数据库，关闭即释放；如需持久化请使用导出功能。
- **开源前自查建议**：不要把自己的数据文件、导出结果、包含账号信息的截图等提交到仓库。
- **仓库图片资产**：本仓库自带的示例图片未发现明显 GPS/作者等元数据字段（已做基础检查）。
